...
public class SharedDataService
{
    ...
    {
        if (string.IsNullOrEmpty(key) || updateFunc == null)
        {
            return default;
        }
        ...
        try
        {
            return (T?)_data.AddOrUpdate(key, 
                k => updateFunc(default), 
                (k, oldValue) => updateFunc(oldValue is T ? (T)oldValue : default));
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "更新数据异常: Key={Key}, Type={Type}", key, typeof(T).Name);
            return default;
        }
    }
} 
...
def on_camera_command(topic: str, data, msg):
  try:
    ...
    if direction == "IN" or direction == "OUT":
      ...
      camera_mqtt.publish("ios/v1/vision/height/result", min_z)
  except Exception as e:
    logger.exception(f"数据非json格式: {e}")

def on_detect_command(topic: str, data, msg):
  try:
    direction = data["data"]["Direction"]
    logger.info(f"收到调度器指令: {topic} -> {direction}")
    if direction == "IN" or direction == "OUT":
...
if __name__ == "__main__":
  ...
  try:
    ...
    camera_mqtt.subscribe("ios/v1/vision/height", qos=MqttQos.EXACTLY_ONCE, callback=on_camera_command)
    camera_mqtt.subscribe("ios/v1/vision/camera/detect", qos=MqttQos.EXACTLY_ONCE, callback=on_detect_command)
    logger.info("成功订阅主题: ios/v1/vision/height")
